...
- `src/components/WorkspaceChat/AgentSidePanel/` — the agent side panel: Changes (aggregated file diffs), Plan (todo stepper), Sessions (terminal + subagent rows with expandable output), Trajectory (per-LLM-iteration debug records, session-only), Sources, and the file reader.
