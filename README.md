...
- **Agent-grade chat UX** — ZCode/Trae-style: live activity chains and run summaries, terminal rows with command-class chips (Search/Run/Install/Write/Fetch…) that expand inline — repeated identical runs fold into "N similar runs" groups — file cards for shell-made changes, a left-edge jump rail that scrolls long threads turn-by-turn, and an agent side panel with Changes, Plan (the live todo list), Sessions (terminal + subagent output), Trajectory (per-turn model requests for debugging local models, repeated iterations folded into ranges), Sources, and a VSCode-style file reader. File names in replies render as chips that open that file in the reader, and API routes in prose render as method badges. Per-chat tool permission modes (ask / auto-approve) and a live context-usage ring round it out.
