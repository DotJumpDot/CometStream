...
- **A real agent harness** — batched tool calls (the model can create 10 files in one reasoned turn, desktop-coding-agent style), an opt-in terminal skill so agents can build, serve, and self-test with shell commands (jailed to a working root, timeout + output caps, host-wrecking command denylist), subagents via `delegate-task`, a per-response tool budget, and prompts sent mid-run queue up and dispatch in order when the run settles.
