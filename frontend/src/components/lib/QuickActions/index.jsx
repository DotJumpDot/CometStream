...
function QuickActionButton({ label, onClick, show = true }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="px-4 py-2 rounded-full bg-theme-bg-chat-input text-white/80 text-sm font-normal leading-5 hover:bg-zinc-700 light:hover:bg-black/20 transition-[background-color,transform] duration-150 active:scale-[0.97] light:text-theme-text-primary"
    >
      {label}
    </button>
  );
